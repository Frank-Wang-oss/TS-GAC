...
class Load_Training_Dataset(Dataset):
    ...
    def __init__(self, dataset, configs, args):
        ...
        if configs.wavelet_aug:
            ...
            X_train_aug1 = wavelet_transform(X_train_aug1, coeffi=configs.augmentation.wavelet_aug_coeffi_weak)
            X_train_aug2 = wavelet_transform(X_train_aug2, coeffi=configs.augmentation.wavelet_aug_coeffi_strong)
    ...
    def augmentation(self, input, weak):
        ...
        input = torch.transpose(input, 1, 3) ### (bs, feature_dimension, num_nodes, time_length)
        # input = torch.transpose(input, 1, 2) ### (bs, num_nodes, feature_dimension, time_length)
        input = torch.reshape(input, [bs, num_nodes* feature_dimension, time_length])
        # input = input.numpy()
        ...
        input = torch.reshape(input, [bs, feature_dimension, num_nodes, time_length])
